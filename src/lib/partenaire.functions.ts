...
export const loginPartenaireFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (!compte) return { ok: false as const, raison: "identifiants_invalides" }

    // TODO: remplacer par bcrypt.compare(data.password, compte.password_hash)
    if (compte.password_hash !== data.password) {
      return { ok: false as const, raison: "identifiants_invalides" }
    }

    return { ok: true as const, partenaire_id: compte.id, nom: compte.nom, email: compte.email }
  })

export const getPartenaireStatsFn = createServerFn({ method: "POST" })
  .inputValidator((input) => z.object({ partenaire_id: z.string().uuid() }).parse(input))
  .handler(async ({ data }) => {
...
export const getLeadDetailFn = createServerFn({ method: "POST" })
  .inputValidator((input) =>
    z.object({ partenaire_id: z.string().uuid(), lead_id: z.string().uuid() }).parse(input),
  )
  .handler(async ({ data }) => {
    const ok = await verifierAssignment(data.partenaire_id, data.lead_id)
